...
## Админ-инструменты и панели

- `GET /about` — страница информации о проекте (рендерит этот README).
- `GET /telescope` — панель Laravel Telescope (локально; запросы, логи, запросы к БД и т.п.).
- `GET /horizon` — панель Laravel Horizon (мониторинг и управление очередями Redis).

В боевых окружениях доступ к Telescope/Horizon должен быть ограничен (по окружению и/или Gate/аутентификации).
